...
### [2026-06-10] v0.5.x — 中央设备管路结构纠偏

- 双漏斗上方新增左右两根不锈钢上接硬管，与顶部总成形成连接关系。
- 删除/停用左右黑色阀门外侧继续延伸到侧壁/后壁的白色管道。
- 双漏斗下方两根连接管改为黑灰硬质弯管，分别连接左右黑色阀门管线。
- 仅保留必要法兰/卡箍感，未新增多余小环扣。
- 手套孔、主体罐体、升降小门和 `right_vessel` 弃用状态未改。

---

### [2026-06-11] v0.5.x — 中部总成真实性纠偏

- 后壁圆形端口、白色圆片和后壁软管占位已从场景停用，背板保持干净。
- 顶部共享大圆盘已移除，保留黑色中心块；双漏斗上方改为两个独立粗上接管。
- 漏斗下方两根管改为与漏斗同色系的不锈钢硬弯管，连接左右黑色阀门区域。
- 左右阀门外侧延伸管/残留接头继续清理，仅保留必要内侧连接。
- 手套孔参数和 `right_vessel` 弃用状态未改；未新增人物、动作或烟雾粒子。

---

### [2026-06-11] v0.5.x — 顶部/漏斗下硬管最终小修

- 顶部共享大盖视觉继续弱化：缩短横杆，两个上接粗管各自保留独立短法兰。
- 漏斗下方左右管改为银灰不锈钢短直段 + 紧凑硬弯头 + 横向接阀门。
- 后壁圆点、阀门外侧白管、手套孔和 `right_vessel` 状态均未改。

### [2026-06-11] v0.5.x — 最终硬管结构修正

- 上部左右粗管继续加粗，直径接近漏斗上口视觉比例，并保留各自短法兰。
- `funnel_lower_hard_pipe_left/right` 改为组件式硬管：短竖直段、紧凑 90° 弯头、水平短管。
- 未改顶部整体、后壁、手套孔、工作台小件、升降小门或 `right_vessel` 状态。

### [2026-06-11] v0.5.x — 最终管道连续性修复

- `funnel_lower_hard_pipe_left/right` 改为每侧一根连续 TubeGeometry，锚点从漏斗下颈到阀门内侧水平管。
- 去除上一轮分段圆柱/弯头/水平管拼接，避免正视和侧视断点。
- 顶部、后壁、手套孔、工作台小件、升降小门和 `right_vessel` 状态未改。

### [2026-06-11] v0.5.x — 管路高度与避让关系修正

- 左右黑色阀门、横向阀门管和内侧接头整体上移到中央主罐上盖/法兰附近。
- 漏斗下硬管路径改为先高位转出，再沿 `centerVesselAvoidRadius + clearance` 外侧接入阀门。
- 管路锚点记录避让半径与间隙，避免穿过中央主罐、上盖和中心短柱。
- 顶部粗管、后壁、手套孔、工作台小件、升降小门和 `right_vessel` 状态未改。

### [2026-06-11] v0.5.x — 管道空间走向最终修正

- 左右阀门与横向管在保持高度的同时前移到主罐体侧前方。
- 漏斗下硬管锚点增加 `centerVesselPipeFrontOffset`，高位向外侧和前侧绕开主罐后再接阀门。
- 顶部粗管、后壁、手套孔、小件、升降门和 `right_vessel` 状态未改。

### [2026-06-11] v0.5.x — 漏斗下方硬管最终纠偏

- 取消漏斗下硬管明显前绕路径，阀门管 Z 位收回到接近漏斗主体平面。
